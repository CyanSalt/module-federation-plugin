...
const getLazyOnceValueSource = (value, lazyOnce) => {
  if (!lazyOnce) return value
...
const getSourceForGlobalVariableExternal = (variableName, globalObject, externalRequest, lazyOnce) => {
  ...
  return Template.asString([
    ...
    Template.indent([
      `? ${getLazyOnceValueSource(`${globalObject}["${externalRequest}"].get(${objectLookup})`, lazyOnce)}`,
...
const getExternalSourceForScriptExternal = (variableName, externalRequest, lazyOnce) => {
  if (!Array.isArray(variableName)) {
    // make it an array as the look up works the same basically
    variableName = [variableName]
  }
  ...
  return Template.asString([
    ...
    `.then(${returningFunction(getLazyOnceValueSource(`${globalName}.get(${objectLookup})`, lazyOnce))});`,
    `${RuntimeGlobals.makeNamespaceObject}(module.exports);`,
    '}());',
  ])
}
...
class RemoteModule extends Module {
   ...
   */
  constructor(request, externalRequest, internalRequest, remoteType, lazyOnce) {
    super('javascript/dynamic', null)
    this.request = request
    this.externalRequest = externalRequest
    this.internalRequest = internalRequest
    this.remoteType = remoteType
    this.lazyOnce = lazyOnce
